...
class HighwayEnv(AbstractEnv):
    ...
    def _reward(self, action: Action) -> float:
        ...
        # may need to utilize lane_distance_to
...
class MOHighwayEnv(HighwayEnv):
    ...
    def _rewards(self, action: Action) -> dict:
        ...
        forward_speed = self.vehicle.speed * np.cos(self.vehicle.heading)
        scaled_speed = utils.lmap(forward_speed, self.config["reward_speed_range"], [0, 1])
        #scaled_speed = self.vehicle.position / forward_speed # --- Original 
        rewards["high_speed"] = ( self.vehicle.speed - 20 ) / (30-20)
        
        #### THIS IS THE PART TO CHANGE ##########################################################
        #  rewards['high_speed"] = np.clip(scaled_speed, 0, 1) # Reward 1 
        #  rewards["high_speed"] = self.vehicle.speed_index / (self.vehicle.target_speeds.size - 1) #Reward 2
        #  rewards["high_speed"] = self.vehicle.speed * np.cos(self.vehicle.heading) *2 #Reward 3
        #  rewards["high_speed"] = ( self.vehicle.speed - 20 ) / (30-20) #Reward 4 
